...
describe('AsyncOptionField Component', () => {
  ...
  describe('optionRenderer Function', () => {
    it('renders the option code and name', () => {
      const option = shallow(optionRenderer({ code: 'a', name: 'b' }));
      // expect(option.containsMatchingElement(<span>a - b</span>)).toBe(true);
      option.unmount();
    });
    it('conditionally renders the region', () => {
      const option = shallow(
        optionRenderer({ name: 'a', region: 'b', country: 'c' })
      );
      // expect(
      //   option.containsMatchingElement(
      //     <span className="option-subtext">b, c</span>
      //   )
      // ).toBe(true);
      option.setProps({ region: null });
      // expect(
      //   option.containsMatchingElement(
      //     <span className="option-subtext">c</span>
      //   )
      // ).toBe(true);
    });
  });
  describe('filterBySearch Function', () => {
    it('filters based on the search', () => {
      expect(filterBySearch([{ name: 'abc' }, { name: 'abd' }], 'abc')).toEqual(
        [{ name: 'abc' }]
      );
    });
  });
